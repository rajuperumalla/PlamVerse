  ...
  const handleDownloadPdf = () => {
    ...
    if (reportElement) {
      ...
      html2canvas(reportElement, { 
        ...
        backgroundColor: 'white', // Ensure a white background for PDF consistency
      }).then(canvas => {
        ...
        // For multi-page PDF:
        const pageHeight = finalPdfHeight > (pdfPageHeight - 2 * margin) ? (pdfPageHeight - 2 * margin) : finalPdfHeight;
        let heightLeft = finalPdfHeight;
        let position = yOffset;

        pdf.addImage(imgData, 'PNG', xOffset, position, finalPdfWidth, finalPdfHeight);
        heightLeft -= pageHeight;

        while (heightLeft > 0) {
          position = position - pageHeight; // Should be negative to capture next part of image
          pdf.addPage();
          pdf.addImage(imgData, 'PNG', xOffset, position, finalPdfWidth, finalPdfHeight);
          heightLeft -= pageHeight;
        }
        ...
        <div className="relative z-10">
            ...
            <ScrollArea className="h-[400px] w-full rounded-md border p-0 bg-background shadow-inner">
                ...
                {reportParagraphs.length > 0 ? (
                    ...
                    <p key={index} className="mb-4 text-base leading-relaxed font-body animate-slide-in-up text-gray-700" style={{animationDelay: `${index * 0.1}s`}}>
                        {paragraph}
                    </p>
                    ))
                ) : (
