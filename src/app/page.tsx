...
export default function LandingPage() {
  return (
    <div className="flex flex-col items-center py-8 md:py-12 space-y-12 md:space-y-20"> {/* Increased bottom spacing */}
      {/* Section 1: Palm Reading - Primary Focus */}
      <section id="palm-reading-auth" className="w-full flex flex-col items-center max-w-md px-4">
        <AuthOptions />
      </section>

      {/* Section 2: Spiritual Products Showcase - Secondary Focus */}
      <section id="spiritual-products" className="w-full max-w-5xl px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {productCategories.map((category) => (
            <Card key={category.name} className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col bg-card">
              <div className="relative h-56 w-full"> {/* Increased image height */}
                <Image
                  ...
                  className="transition-transform duration-300 group-hover:scale-105" // Added group for hover effect if Card is a group
                />
              </div>
              ...
              <CardContent className="p-5 pt-0 flex-grow"> {/* Adjusted padding */}
                <p className="text-muted-foreground text-sm md:text-base">{category.description}</p>
              </CardContent>
              <div className="p-5 border-t border-border"> {/* Ensured button is at the bottom with padding */}
